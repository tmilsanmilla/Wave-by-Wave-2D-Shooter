-- OUTPOST ZERO / ADMINISTRATION / 04: USERNAME ACTIONS + TESTER ADMIN TOOLS
-- Run after Administration 01–03 and Social 01. Safe to rerun.
--
...
-- ---------------------------------------------------------------------------
-- TESTER ROLE, WEAPON SUGGESTIONS, DEMOTION, AND REPORT COPY
-- ---------------------------------------------------------------------------
...
begin
  if to_regclass('public.admins') is null
    ...
    raise exception 'Administration 04 requires Administration 01 and 02';
  end if;
  if to_regclass('public.admin_msgs') is null or to_regclass('public.reports') is null then
    raise exception 'Administration 04 requires the original admin_msgs and reports tables';
  end if;
end;
$preflight$;
