...
function cpuTeamBeginRound(options){
  ...
  partyCpuMatch.roundResolved=false;partyCpuMatch.phase='countdown';partyCpuMatch.seenHits=new Set();partyCpuMatch.seenShots=new Set();partyCpuMatch.seenPlayerShots=new Set();partyCpuMatch.seenReflections=new Set();partyCpuMatch.seenReflectionHits=new Set();
...
const PARTY_CPU_WEAPON_RULES=Object.freeze({
  ar:Object.freeze({damage:18,fireMs:WEAPONS.ar.fireRate*1.68,maxRange:1050}),
  m9:Object.freeze({damage:24,fireMs:WEAPONS.m9.fireRate*1.68,maxRange:440}),
  knife:Object.freeze({damage:42,fireMs:WEAPONS.knife.fireRate,maxRange:WEAPONS.knife.range}),
});
function partyCpuWeaponRule(key){return PARTY_CPU_WEAPON_RULES[key]||PARTY_CPU_WEAPON_RULES.ar;}
function partyCpuSpawnBotShot(bot,target,profile,weaponId=bot&&bot.cur){
  if(!cpuTeamIsAuthority()||partyCpuMatch.phase!=='fight') return;
  weaponId=['ar','m9'].includes(String(weaponId||''))?String(weaponId):'ar';
  const w=WEAPONS[weaponId],rule=partyCpuWeaponRule(weaponId);
  ...
  const a=bot.angle+jitter,speed=weaponBulletSpeed(weaponId),id=partyCpuMatch.epoch+':'+partyCpuMatch.round+':shot:'+(++partyCpuMatch.shotSeq);
  const shot={id,ownerId:bot.id,team:bot.team,targetId:target&&target.id||'',x:bot.x+Math.cos(a)*7,y:bot.y+Math.sin(a)*7,
    vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,life:weaponBulletLife(weaponId,1200),dmg:rule.damage,dist:0,rng:w.range,fall:w.fall,weapon:weaponId};
...
function partyCpuDodgeShots(bot){
  if(!bot)return [];
  const shots=[];
  for(const shot of Array.isArray(partyCpuMatch.shots)?partyCpuMatch.shots:[]){
    if(shot&&String(shot.team||'')!==String(bot.team||'')&&
       (!shot.targetId||String(shot.targetId)===String(bot.id||'')))shots.push(shot);
  }
  // Human projectiles use the normal local bullet simulation for the host and
  // validated visual tracers for the remote teammate. Only red CPUs consider
  // these team-A shots hostile; the ally CPU never dodges friendly fire.
  if(bot.team==='B'){
    if(Array.isArray(bullets))shots.push(...bullets);
    if(Array.isArray(partyCpuMatch.visualShots))shots.push(...partyCpuMatch.visualShots);
  }
  return shots;
}
function partyCpuHostStep(dtms,clock){
  ...
  for(const b of partyCpuMatch.bots){
    ...
    const weaponKey=String(b.cur||CPU_AI_LOADOUT.primary),w=WEAPONS[weaponKey]||WEAPONS.ar,weaponRule=partyCpuWeaponRule(weaponKey),
      ...
      shotProfile={damage:weaponRule.damage,rng:w.range||1,fall:w.fall||1,maxRange};
    if(targetVisible&&(!localMatch||profile.usePrediction))cpuAiTrackTarget(b,target,dtms,clock);
    if(!isMelee&&(!localMatch||profile.useTnt)&&clock>=(b.tntThinkAt||0)){
    ...
    const lead=isMelee?{x:target.x,y:target.y,leadMs:0,visible:cpuAiMeleeClear(b,target)}:
      (targetVisible?cpuAiLeadPoint(b,target,weaponKey,!localMatch||profile.usePrediction?profile.leadFactor:0,!localMatch||profile.usePrediction?profile.maxLeadMs:0):null),
      rangedAim=isMelee?null:cpuAiRangedAimSolution(b,target,clock,profile,lead,targetVisible),liveTnt=activeArenaTnt(),
    ...
    if((+b.aiWeaponMags?.[weaponKey]||0)<=0){b.aiWeaponThinkAt=clock;cpuAiStartBotReload(b,weaponKey,clock);continue;}
    const interval=weaponRule.fireMs,previous=Math.max(0,+b.aiWeaponLastShot?.[weaponKey]||0);if(clock-previous<interval)continue;
    const fireDecision=cpuAiPressureFireDecision(b,target,clock,profile,{available:aimTntId?true:rangedAim.available,
      visible:aimTntId?true:rangedAim.visible,aimErr,holdRanged:parryResponse.holdRanged,tnt:!!aimTntId});
    if(!fireDecision.fire)continue;
    const shotStamp=clock;
    if(!cpuAiSpendBotRound(b,weaponKey,shotStamp))continue;cpuAiRecordPressureShot(b,fireDecision);b.flash=clock+55;
    partyCpuSpawnBotShot(b,aimTntId?{id:aimTntId,kind:'tnt'}:target,profile,weaponKey);
    if((+b.aiWeaponMags[weaponKey]||0)<=0)b.aiWeaponThinkAt=clock;
  }
}
...
function partyCpuHostTryParry(target,shot,incomingDamage){
  if(!cpuTeamIsAuthority()||!target||!shot||shot.melee===true) return false;
  const originalId=String(shot.id||'');
  if(!originalId)return false;
  if(!(partyCpuMatch.seenReflections instanceof Set))partyCpuMatch.seenReflections=new Set();
  // A reflected authority shot is consumed forever. Replaying its original ID
  ...
  const source=partyCpuMatch.bots.find(b=>b.id===String(shot.ownerId||'')&&b.team==='B'),
    reflectionDamage=clamp(+incomingDamage||0,0,PARTY_CPU_HP);
  const interceptX=+shot.x,interceptY=+shot.y;
  if(!source||!reflectionDamage||!Number.isFinite(interceptX)||!Number.isFinite(interceptY))return false;
  partyCpuMatch.seenReflections.add(originalId);
  if(partyCpuMatch.seenReflections.size>500)partyCpuMatch.seenReflections=new Set([...partyCpuMatch.seenReflections].slice(-250));
  const rawAngle=local?aimAngle():+target.angle,
    angle=Number.isFinite(rawAngle)?Math.atan2(Math.sin(rawAngle),Math.cos(rawAngle)):0,
    reflectedShot={id:originalId+':reflect',originalShotId:originalId,reflected:true,parrySeq:Math.max(0,Math.floor(local?parrySeq:target.parrySeq||0)),
      ownerId:String(target.id||''),sourceId:source.id,team:'A',targetId:'',x:interceptX,y:interceptY,
      vx:Math.cos(angle)*22,vy:Math.sin(angle)*22,
      life:1800,dmg:reflectionDamage,dist:0,rng:3000,fall:1};
  partyCpuMatch.shots.push(reflectedShot);
  if(!isLocalCpu2v2()){
    const reflectedPacket={matchEpoch:partyCpuMatch.epoch,hostEpoch:partyCpuMatch.hostEpoch,round:partyCpuMatch.round,
      id:reflectedShot.id,shotId:originalId,ownerId:reflectedShot.ownerId,sourceId:source.id,
      parrySeq:reflectedShot.parrySeq,x:reflectedShot.x,y:reflectedShot.y,angle,dmg:reflectionDamage};
    partySend('cpu_reflected_shot',reflectedPacket);
    for(const wait of [180,520])setTimeout(()=>{
      if(party.channel&&partyCpuSessionOpen()&&partyCpuMatch.epoch===reflectedPacket.matchEpoch&&partyCpuMatch.round===reflectedPacket.round)
        partySend('cpu_reflected_shot',reflectedPacket);
    },wait);
  }
  if(!local&&!isLocalCpu2v2()){
    const packet={to:target.id,matchEpoch:partyCpuMatch.epoch,hostEpoch:partyCpuMatch.hostEpoch,round:partyCpuMatch.round,
      id:originalId+':parry',shotId:originalId,dmg:0,hp:target.hp,parried:true,
      sourceId:source.id,reflected:0};
...
function partyCpuApplyDamage(p){
  ...
  if(p.parried){
    ...
    const reflected=clamp(+p.reflected||0,0,PARTY_CPU_HP); if(source&&reflected) addDamageNumber(source,reflected,true);
      ...
      if(cpuTeamIsAuthority()){
        ...
        if(!dead)for(const target of partyCpuMatch.bots){
          ...
          if(dist2(b.x,b.y,target.x,target.y)<(target.r+4)*(target.r+4)){
            ...
            if(b.reflected){
              const reflectionOwner=String(b.ownerId||''),localOwner=String(cpuTeamLocalId()||'');
              if(reflectionOwner===localOwner)addDamageNumber(target,dealt,true);
              else if(!isLocalCpu2v2()&&partyCpuMatch.humanIds.includes(reflectionOwner)){
                const packet={to:reflectionOwner,matchEpoch:partyCpuMatch.epoch,hostEpoch:partyCpuMatch.hostEpoch,round:partyCpuMatch.round,
                  id:String(b.id)+':hit:'+String(target.id),shotId:String(b.id),ownerId:reflectionOwner,targetId:String(target.id),dmg:dealt};
                partySend('cpu_reflection_hit',packet);for(const wait of [180,520])setTimeout(()=>{
                  if(party.channel&&partyCpuSessionOpen()&&partyCpuMatch.epoch===packet.matchEpoch&&partyCpuMatch.round===packet.round)
                    partySend('cpu_reflection_hit',packet);
                },wait);
              }
            }
...
function partyCpuApplyPlayerState(p){
  ...
  if(Number.isFinite(+p.angle))h.angle=Math.atan2(Math.sin(+p.angle),Math.cos(+p.angle));
  ...
  arenaApplyRemoteMeleeAbilityState(h,p,kit,Date.now());
...
function partyCpuApplyBotSnapshot(p){
  ...
  for(const raw of p.bots.slice(0,3)){
    ...
    if(swingSeq>Math.max(0,Math.floor(+b.swingSeq||0))){
      b.swingSeq=swingSeq;
      if(remoteKey===kit.melee&&swingMs>0&&Number.isFinite(+raw.swingA)){
        b.swingDur=clamp(WEAPONS[kit.melee].fireRate*.55,90,260);
        b.swingT=cpuTeamClock()-(b.swingDur-Math.min(swingMs,b.swingDur));
        b.swingA=Math.atan2(Math.sin(+raw.swingA),Math.cos(+raw.swingA));b.swingArc=WEAPONS[kit.melee].arc;
        b.swingR=WEAPONS[kit.melee].range;b.swingSide=raw.swingSide===-1?-1:1;
      }
    }
...
function partyCpuApplyBotShot(p){
  ...
  const weapon=String(s.weapon||'ar'),kit=cpuAiBotLoadout(owner),w=WEAPONS[weapon],rule=partyCpuWeaponRule(weapon);
  if(!w||w.melee||![kit.primary,kit.secondary].includes(weapon))return false;
  const nums=['x','y','vx','vy','life','dmg','dist','rng','fall'];if(nums.some(k=>!Number.isFinite(+s[k])))return false;
  const expectedSpeed=weaponBulletSpeed(weapon),maxLife=weaponBulletLife(weapon,1200);
  if(Math.abs(Math.hypot(+s.vx,+s.vy)-expectedSpeed)>.03||+s.life<=0||+s.life>maxLife+.01||
     Math.abs(+s.dmg-rule.damage)>.001||Math.abs(+s.rng-w.range)>.001||Math.abs(+s.fall-w.fall)>.001)return false;
  ...
  partyCpuMatch.shots.push({id,ownerId:owner.id,team:owner.team,targetId:String(s.targetId||''),x:clamp(+s.x,ARENA_EDGE,WORLD.w-ARENA_EDGE),y:clamp(+s.y,ARENA_EDGE,WORLD.h-ARENA_EDGE),
    vx:+s.vx,vy:+s.vy,life:clamp(+s.life,1,maxLife),dmg:rule.damage,dist:Math.max(0,+s.dist),rng:w.range,fall:w.fall,weapon});return true;
}
function partyCpuApplyReflectedShot(p){
  if(!partyCpuEnvelope(p,true)||partyCpuIsHost()||p.round!==partyCpuMatch.round||partyCpuMatch.phase!=='fight')return false;
  const originalId=String(p.shotId||''),id=String(p.id||''),prefix=partyCpuMatch.epoch+':'+partyCpuMatch.round+':shot:',
    ownerId=String(p.ownerId||''),sourceId=String(p.sourceId||''),owner=partyCpuMatch.humans[ownerId],source=partyCpuMatch.bots.find(b=>b.id===sourceId&&b.team==='B'),
    parrySequence=+p.parrySeq,x=+p.x,y=+p.y,rawAngle=+p.angle,damage=+p.dmg;
  if(!originalId.startsWith(prefix)||!/^[1-9]\d{0,9}$/.test(originalId.slice(prefix.length))||id!==originalId+':reflect'||id.length>120||
     !owner||owner.team!=='A'||!source||!Number.isSafeInteger(parrySequence)||parrySequence<1||parrySequence>1000000000||
     !partyCpuMatch.loadouts[ownerId]||partyCpuMatch.loadouts[ownerId].melee!=='twinsai'||
     !Number.isFinite(x)||!Number.isFinite(y)||!Number.isFinite(rawAngle)||Math.abs(rawAngle)>Math.PI+1e-6||
     !Number.isFinite(damage)||damage<=0||damage>PARTY_CPU_HP)return false;
  const localOwner=ownerId===String(cpuTeamLocalId()||''),knownParrySeq=localOwner?Math.max(0,Math.floor(parrySeq||0)):Math.max(0,Math.floor(owner.parrySeq||0));
  if(localOwner?parrySequence!==knownParrySeq:parrySequence<knownParrySeq)return false;
  const bounds=activeArenaBounds();
  if(x<bounds.left||x>bounds.right||y<bounds.top||y>bounds.bottom)return false;
  const actorPoints=localOwner?[[player.x,player.y],[owner.x,owner.y]]:[[owner.x,owner.y],[owner.tx,owner.ty]],
    nearOwner=actorPoints.some(([ox,oy])=>Number.isFinite(+ox)&&Number.isFinite(+oy)&&Math.hypot(x-ox,y-oy)<=160);
  if(!nearOwner)return false;
  const original=partyCpuMatch.shots.find(s=>String(s.id||'')===originalId);
  if(original&&(String(original.ownerId||'')!==sourceId||original.team!=='B'||damage>(+original.dmg||0)+1e-6))return false;
  if(!(partyCpuMatch.seenReflections instanceof Set))partyCpuMatch.seenReflections=new Set();
  if(partyCpuMatch.seenReflections.has(originalId))return false;
  partyCpuMatch.seenReflections.add(originalId);
  if(partyCpuMatch.seenReflections.size>500)partyCpuMatch.seenReflections=new Set([...partyCpuMatch.seenReflections].slice(-250));
  const angle=Math.atan2(Math.sin(rawAngle),Math.cos(rawAngle));
  partyCpuMatch.shots=partyCpuMatch.shots.filter(s=>String(s.id||'')!==originalId);
  partyCpuMatch.shots.push({id,originalShotId:originalId,reflected:true,parrySeq:parrySequence,ownerId,sourceId,team:'A',targetId:'',x,y,
    vx:Math.cos(angle)*22,vy:Math.sin(angle)*22,
    life:1800,dmg:damage,dist:0,rng:3000,fall:1});
  return true;
}
function partyCpuApplyReflectionHit(p){
  if(!partyCpuEnvelope(p,true)||p.round!==partyCpuMatch.round||!['fight','round_end'].includes(partyCpuMatch.phase)||
     String(p.to||'')!==String(cpuTeamLocalId()||'')||String(p.ownerId||'')!==String(cpuTeamLocalId()||''))return false;
  const shotId=String(p.shotId||''),targetId=String(p.targetId||''),id=String(p.id||''),
    prefix=partyCpuMatch.epoch+':'+partyCpuMatch.round+':shot:',dmg=+p.dmg,
    target=partyCpuMatch.bots.find(b=>String(b.id||'')===targetId&&b.team==='B');
  if(!shotId.startsWith(prefix)||!shotId.endsWith(':reflect')||shotId.length>130||id!==shotId+':hit:'+targetId||id.length>180||
     !target||!partyCpuMatch.loadouts[p.ownerId]||partyCpuMatch.loadouts[p.ownerId].melee!=='twinsai'||
     !Number.isFinite(dmg)||dmg<=0||dmg>50)return false;
  if(!(partyCpuMatch.seenReflectionHits instanceof Set))partyCpuMatch.seenReflectionHits=new Set();
  if(partyCpuMatch.seenReflectionHits.has(id))return false;
  partyCpuMatch.seenReflectionHits.add(id);
  if(partyCpuMatch.seenReflectionHits.size>200)partyCpuMatch.seenReflectionHits=new Set([...partyCpuMatch.seenReflectionHits].slice(-100));
  partyCpuMatch.shots=partyCpuMatch.shots.filter(shot=>String(shot.id||'')!==shotId);
  addDamageNumber(target,dmg,true);return true;
}
...
function partyCpuReceive(event,p){
  ...
  if(event==='cpu_reflected_shot'){partyCpuApplyReflectedShot(p);return;}
  if(event==='cpu_reflection_hit'){partyCpuApplyReflectionHit(p);return;}
...
let profileExitFlushStarted=false;
function flushProfileOnExit(){
  if(profileExitFlushStarted||typeof profileLoaded==='undefined'||!profileLoaded||
     typeof profileOwnerUserId==='undefined'||!authUser||profileOwnerUserId!==String(authUser.id||'')||
     typeof saveProfile!=='function')return false;
  profileExitFlushStarted=true;void saveProfile(true);return true;
}
addEventListener('pagehide',event=>{
  flushLayoutDraftOnExit();
  if(!event||event.persisted!==true)flushProfileOnExit();
  try{if((!event||event.persisted!==true)&&typeof handleBotLadderPageExit==='function')handleBotLadderPageExit(event);}catch(e){}
  try{if((!event||event.persisted!==true)&&typeof arenaForfeitOnPageExit==='function')arenaForfeitOnPageExit();}catch(e){}
});
