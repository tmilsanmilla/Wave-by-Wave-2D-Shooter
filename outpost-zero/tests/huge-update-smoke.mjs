...
const admin01Sql=read('sql/administration/Admin-01-admin-menu.sql');
...
check('Leaderboards owns score security and Realtime',/alter table public\.scores force row level security/.test(leaderboardSecuritySql)&&/add table public\.scores/.test(leaderboardSecuritySql));
check('Weapons owns price and definition security plus Realtime',/alter table public\.weapon_prices force row level security/.test(weaponsSql)&&/alter table public\.weapon_defs force row level security/.test(weaponsSql)&&/array\['weapon_prices', 'weapon_defs'\]/.test(weaponsSql));
